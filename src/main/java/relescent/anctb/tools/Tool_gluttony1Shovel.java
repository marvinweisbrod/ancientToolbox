...
public class Tool_gluttony1Shovel extends Tool_toolShovel{

	public Tool_gluttony1Shovel(ToolMaterial material) {
		super(material);

		this.setCreativeTab(AddedElse.tabAncTB);
		this.setTextureName(Reference.MOD_ID + ":" + UnlocNames.tool_gluttony1Shovel);
	}
    ...
    public boolean onBlockStartBreak(ItemStack itemstack, int x, int y, int z, EntityPlayer player)
    {
		...
		if(block == Blocks.dirt || block == Blocks.grass){
			...
			if (!player.getEntityWorld().isRemote) {
				player.getEntityWorld().func_147480_a(x, y, z, false);
			}
			return true;
		}
        return false;
    ...
    {
    	ItemStack newTool = new ItemStack(AddedItems.tool_gluttony1Pick);
